program density_check
        ...
        do i = 1, N1
           do j = 1, N2
              do k = 1, N3
                 rho_x(i) = rho_x(i) + phi((l-1)*N1*N2*N3 + (i-1)*N2*N3 + (j-1)*N3 + k)&
                         *CONJG(phi((l-1)*N1*N2*N3 + (i-1)*N2*N3 + (j-1)*N3 + k))
              end do
           end do
        end do
        end do

        do l = 1, HO_index-low_index ! Density only includes occ. orbitals
        do i = 1, N1 ! Loop over possible x values
              do j = 1, N2 ! Loop over possible y values
                  do k = 1, N3 ! Loop over possible z values

                      rho_x(i) = rho_x(i) + phi((l-1)*N1*N2*N3 + (i-1)*N2*N3 + (j-1)*N3 + k) * &
                              CONJG(phi((l-1)*N1*N2*N3 + (i-1)*N2*N3 + (j-1)*N3 + k))
                  
                      rho_y(j) = rho_y(j) + phi((l-1)*N1*N2*N3 + (i-1)*N2*N3 + (j-1)*N3 + k) * &
                              CONJG(phi((l-1)*N1*N2*N3 + (i-1)*N2*N3 + (j-1)*N3 + k))
                  
                      rho_z(k) = rho_z(k) + phi((l-1)*N1*N2*N3 + (i-1)*N2*N3 + (j-1)*N3 + k) * &
                              CONJG(phi((l-1)*N1*N2*N3 + (i-1)*N2*N3 + (j-1)*N3 + k))
                  
                  end do ! phi_j(x,y,z) parameterized by phi(n)
                  end do ! where n = (j-1)*N1*N2*N3 + (x-1)*N2*N3 + (y-1)*N3 + z
        end do
        end do


        do i = 1, N1
           !print*, rho_x(i)/MAXVAL(rho_x)
